...
func _get_super_call_text(method_name, flags, args):
	if flags & MethodFlags.METHOD_FLAG_VIRTUAL_REQUIRED != 0:
		return '__gutdbl.gut_ref.get_ref()._fail("Cannot call super method because it is abstract.")'
	...
	if(method_params != null):
		if(meta.name == '_init'):
			...
			text = _func_text.format({
				...
				"method_name":meta.name,
				"param_array":param_array,
				"super_call":_get_super_call_text(meta.name, meta.flags, args),
			})
